...
class GaussianCell(tf.contrib.rnn.RNNCell):
    """
    RNN cell that  the mean and the std of a Gaussian distribution as a function of the hidden state
    """
...
class GaussianConvolutionalCell(tf.contrib.rnn.RNNCell):
    ...
    def __init__(self, cell, event_size, reuse=None):
        ...
        self._conv_layer1 = tf.layers.Conv2D(8, (5, 5), strides=(1, 1), padding='valid')
        self._conv_layer2 = tf.layers.Conv2D(16, (5, 5), strides=(2, 2), padding='same')
        self._conv_layer3 = tf.layers.Conv2D(32, (5, 5), strides=(2, 2), padding='same')
        self._conv_layer4 = tf.layers.Conv2D(64, (5, 5), strides=(2, 2), padding='same')
        self._conv_layer5 = tf.layers.Conv2D(64, (5, 5), strides=(2, 2), padding='same')
        self._conv_dense = tf.layers.Dense(self._cell.state_size)

        self._deconv_dense1 = tf.layers.Dense(64)
        self._deconv_layer1 = tf.layers.Conv2DTranspose(64, (5, 5), strides=(2, 2), padding='same')
        self._deconv_layer2 = tf.layers.Conv2DTranspose(32, (5, 5), strides=(2, 2), padding='same')
        self._deconv_layer3 = tf.layers.Conv2DTranspose(16, (5, 5), strides=(2, 2), padding='same')
        self._deconv_layer4 = tf.layers.Conv2DTranspose(8, (5, 5), strides=(2, 2), padding='same')
        self._deconv_mean = tf.layers.Conv2D(1, (3, 3), strides=(1, 1), padding='valid')
        self._deconv_std = tf.layers.Conv2D(1, (3, 3), strides=(1, 1), padding='valid', activation=tf.nn.softplus)
...
class SamplingCell(tf.contrib.rnn.RNNCell):
    """
    RNN cell that produce a sequence in autoregressive mode by sampling at each step
    """
